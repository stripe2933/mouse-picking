...
class AppWindow final : public OGLWrapper::GLFW::Window {
    ...
    static constexpr std::size_t num_cube_in_side = 20;
    static constexpr std::size_t num_total_cubes = num_cube_in_side * num_cube_in_side * num_cube_in_side;
    static constexpr std::uint16_t no_hover_index = 0xFFFF;
    static_assert(num_total_cubes < no_hover_index);
    std::uint16_t hovered_index = no_hover_index;

    std::vector<glm::vec3> rotation_axes;
    std::vector<glm::mat4> models;

    // View/projection related properties.
    static constexpr float camera_distance = 30.f;
