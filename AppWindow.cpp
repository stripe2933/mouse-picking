...
void AppWindow::updateImGui(float time_delta) {
    ...
    if (static glm::vec3 outline_color { 1.f, 0.5f, 0.2f }; ImGui::ColorEdit3("Outline color", value_ptr(outline_color))) {
        outliner_instanced_program.pendUniforms([& /* outline_color is in static lifetime: no need to value capture */]() {
            glUniform3fv(outliner_instanced_program.getUniformLocation("color"), 1, value_ptr(outline_color));
        });
    }
...
void AppWindow::draw() const {
    ...
    if (hovered_index != no_hover_index) {
        // If there is a mesh under the cursor (retrieved by glReadPixel), draw it with slightly scaled model.
